...
# Summarize convergence messages for MLE
res |> 
  filter(sim_msg == "100% PPV (Queried)") |> 
  group_by(N, ppv) |> 
  summarize(num = n())

# Exclude reps where 100% PPV in the queried subset
res = res |> 
  filter(!(!is.na(sim_msg) & sim_msg == "100% PPV (Queried)"))
...
# Create boxplot of estimated outcome model parameters
est_box = res |> 
  select(sim, N, beta0, beta1, beta2, ppv, 
                starts_with(c("beta0_", "beta1_", "beta2_"))) |> 
  gather(key = "Coeff_Method", value = "Estimate", -c(1:6)) |> 
  mutate(
    Method = case_when(
...
# Save it 
ggsave(filename = "~/Documents/food_access_misclassification/Figures/try_mlePossum2_with_Z_modYgivXZ.png",
       plot = est_box, 
       device = "png", 
       width = 7, 
       height = 5)

# Create boxplot of estimated misclassification model parameters
est_box = res |> 
  select(sim, N, eta0, eta1, ppv, 
         starts_with(c("eta0_", "eta1_"))) |> 
  gather(key = "Coeff_Method", value = "Estimate", -c(1:5)) |> 
  mutate(
    Method = case_when(
      grepl(x = Coeff_Method, pattern ="gs") ~ "Gold\nStandard",
      grepl(x = Coeff_Method, pattern ="mle") ~ "MLE",
    ...
    Method = factor(x = Method, 
                    levels = c("Gold\nStandard", "MLE", "Naive", "Complete\nCase", "Truth")), 
    Truth = case_when(
      grepl(x = Coeff_Method, pattern ="eta0") ~ eta0,
      grepl(x = Coeff_Method, pattern ="eta1") ~ eta1
    ), 
    Coeff = case_when(
      grepl(x = Coeff_Method, pattern ="eta0") ~ "Intercept",
      grepl(x = Coeff_Method, pattern ="eta1") ~ "Log OR of Z"
    ), 
    Coeff = factor(x = Coeff, 
                   levels = c("Intercept", "Log OR of Z")),
    N = factor(x = N, 
               levels = c(390, 2200), 
               labels = c("N = 390", "N = 2200"))) |> 
  filter(Method %in% c("Gold\nStandard", "MLE", "Truth")) |> 
  ggplot(aes(x = factor(ppv), y = exp(Estimate), fill = Method)) + 
  geom_hline(aes(yintercept = exp(Truth)), 
             linetype = 2, 
             color = "darkgrey") + 
  ...
  theme_minimal(base_size = 20) + 
  xlab(label = latex2exp::TeX(input = "Positive Predictive Value of X*")) + 
  ylab(label = latex2exp::TeX(input = "Estimated Odds [Ratio]")) + 
  theme(legend.position = "right", 
        legend.box.margin = margin(t = 10, r = 5, b = -30, l = 5), 
        plot.margin = margin(t = 0, r = 10, b = 0, l = 10), 
        strip.background = element_rect(fill = slide_colors[2])) 
...
# Save it 
ggsave(filename = "~/Documents/food_access_misclassification/Figures/try_mlePossum2_with_Z_modXgivZ.png",
       plot = est_box, 
       device = "png", 
       width = 7, 
       height = 5)
