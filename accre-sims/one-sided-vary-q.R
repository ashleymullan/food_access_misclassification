...
# Simulation Setting: Varied Query Proportion (One-Sided)
...
repo_string <- "food_access_misclassification/accre-sims/check-me-q/"
...
f_ppv <- 0.6 ## positive predictive value, used to generate eta0
f_eta0 <- log(f_ppv/(1 - f_ppv))
...
for (N in N_list) { ## iterate through sample sizes
    for (q in q_list){ ## iterate through query percentages
      ...
      results <- data.frame( ## initialize results container
        ...
        N, beta0 = f_beta0, beta1 = f_beta1, beta2 = f_beta2, q = q, eta1 = f_eta1, ## simulation setting
        eta0 = f_eta0, ppv = f_ppv, queried_ppv = NA, avg_prev = NA, fpr = NA, ## simulation setting
      ...
      for (r in 1:num_reps) {
        ...
        # Prepare for query
        query_size <- ifelse(test = ceiling(q * N) <= nrow(dat_reduced),
                             yes = ceiling(q * N),
                             no = nrow(dat_reduced))

        query_rows = sample(x = dat_reduced$id, ## can only sample from IDs with X* = 1
                            size = query_size, ## can't exceed amount of data we have
                            replace = FALSE)
        ...
        write.csv(x = results,
                  ...
                  row.names = F)
        toc() ## End runtime for sims with current N and q
    } ## end loop over q
} ## end loop over N
...
files = all_files[grepl("varyQ_", all_files)]
...
write.csv(x = res,
          file = paste0("one-sided-vary-q.csv"),
          row.names = F)
