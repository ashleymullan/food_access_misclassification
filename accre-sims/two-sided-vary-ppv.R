...
sim_data <- function(N, beta0 = f_beta0, beta1 = f_beta1, beta2 = f_beta2,
  ...
  #Simulate true access X|X*,Z
  binX <- rbinom(n = N, size = 1, prob = 1 / (1 + exp(-(eta0 + eta1 * binXstar + eta2 * Z))))
...
for (N in N_list) { ## iterate through sample sizes
    for (ppv in ppv_list){ ## iterate through sample sizes
      ...
      for (r in 1:num_reps) {
        ...
        # split data by Xstar
        dat_xstar <- dat |> dplyr::filter(binXstar == 1)
        dat_noxstar <- dat |> dplyr::filter(binXstar == 0)

        # Prepare for query
        query_rows0 = sample(x = dat_noxstar$id, ## can only sample from IDs with X* = 1
                            size = floor(f_q * N / 2),
                            replace = FALSE)
        query_rows1 = sample(x = dat_xstar$id,
                             size = ceiling(f_q * N / 2),
                             replace = FALSE)
        query_rows <- c(query_rows0, query_rows1)
        ...
        if(queried_ppv != 1) {
          tryCatch({
            fit_mle <- suppressMessages(
              mlePossum(
                analysis_formula = Cases ~ binX + Z + offset(log(P)),
...
files = all_files[grepl("vary2PPV_", all_files)]
