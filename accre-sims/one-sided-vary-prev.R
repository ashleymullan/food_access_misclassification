...
f_ppv <- 0.6 ## positive predictive value
f_eta0 <- log(f_ppv/(1 - f_ppv)) ## compute intercept of error model
...
for (N in N_list) { ## iterate through sample sizes
    for (eb0 in exp_beta0_list){ ## iterate through outcome prevalences
      ...
      for (r in 1:num_reps) {
        ...
        write.csv(x = results,
                  file = paste0("varyprev_N", N, "_prev", 100 * eb0, "_seed", sim_seed, ".csv"),
                  row.names = F)
        toc() ## End runtime for sims with current N and prev
    } ## end loop over prev
} ## end loop over N
...
files = all_files[grepl("varyprev_", all_files)]
...
write.csv(x = res,
          file = paste0("one-sided-vary-prev.csv"),
          row.names = F)
