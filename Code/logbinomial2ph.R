#' Sieve maximum likelihood estimator (SMLE) for two-phase log-binomial regression problems with covariate measurement error
#' This function returns the sieve maximum likelihood estimators (SMLE) for the log-binomial regression model with covariate measurement error from Lotspeich et al. (2024+)
#'
#' @param Y Column name with the outcome 
#' @param offset Column name with the offset for \code{Y}. 
...
logbinomial2ph = function(Y, offset, X_unval, X_val, Z = NULL, Bspline = NULL, data, theta_pred = NULL, initial_lr_params = "Zero", h_N_scale = 1, noSE = FALSE, TOL = 1E-4, MAX_ITER = 1000) {
  # Determine error setting -----------------------------------------
  ## If unvalidated variable was left blank, assume error-free ------
  errorsX = !is.null(X_unval)
  if (errorsX) {
    ### Create indicator for validated rows -------------------------
    data$Validated = as.numeric(!is.na(data[, X_val]))
  } else {
    data$Validated
  }
  ## ------ If unvalidated variable was left blank, assume error-free
  ...
  theta_formula = as.formula(paste0("cbind(", Y, ",", offset, "-", Y, ")~", paste(theta_pred, collapse = "+")))
  if (errorsX) {
    # Save distinct X -------------------------------------------------
  ...
  if(initial_lr_params == "Zero") {
    prev_theta = theta0 = matrix(data = c(log(0.5), rep(0, length(theta_pred))), 
  ...
  while(it <= MAX_ITER & !CONVERGED) {
    ...
    ### P(Y|X) --------------------------------------------------------
    mu_theta = exp(as.numeric((theta_design_mat[-c(1:n), ] %*% prev_theta)))
