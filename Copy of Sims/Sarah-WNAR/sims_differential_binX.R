...
fix_beta2 = 0# -0.1416510  ## log prevalence ratio for Z on Y (given X)
...
# ---------------------------------------------------------------------------------
sim_data = function(N, beta0 = fix_beta0, beta1 = fix_beta1, beta2 = fix_beta2) {
  ## Simulate metropolitan indicator 
  Z = rbinom(n = N, 
             size = 1, 
             prob = 0.85)
  ...
  binXstar = rbinom(n = N, 
                    size = 1, 
                    prob = 1 / (1 + exp(- (-0.5725192 + 0.6584616 * Z))))

  ## Simulate map-based access
  binX = binXstar
  binX[binXstar == 1] = rbinom(n = sum(binXstar == 1), 
                               size = 1, 
                               prob = 1 / (1 + exp(- (1.845827 + -1.415790 * Z))))
...
for (N in c(390, 2200)) {
  ...
  for (q in c(0.1, 0.25, 0.5, 0.75)){
    ...
    sett_res = data.frame(
      ...
      N, beta0 = fix_beta0, beta1 = fix_beta1, ## simulation setting
      q = q, avg_prev = NA, fpr = NA, ppv = NA, ## simulation setting
    ...
    for (r in 1:num_reps) {
      ...
      sett_res$fpr[r] = sum(dat$binX == 1 & dat$binXstar == 1) /
        sum(dat$binXstar == 1)
      ...
      write.csv(x = sett_res,
                file = paste0("diff_errors/proximity_N", N, "_q", 100 * q, "_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
...
# Timing from tictoc:
## Sims with N = 390: 142.33 sec elapsed
## Sims with N = 2200: 501.459 sec elapsed