...
for (N in c(390, 2200)) {
  ...
  for (q in c(0.1, 0.25, 0.5, 0.75)){
    ...
    sett_res = data.frame(
      ...
      beta0_smle = NA, se_beta0_smle = NA, beta1_smle = NA, se_beta1_smle = NA, ppv_smle = NA ## imputation analysis
    )
    ...
    for (r in 1:num_reps) {
      ...
      sett_res$ppv[r] = sum(dat$binX == 1 & dat$binXstar == 1) /
        sum(dat$binXstar == 1)
      ...
      sett_res[r, "ppv_smle"] = fit_smle$ppv
...
# Timing from tictoc:
# Sims with N = 390: 156.96 sec elapsed
# Sims with N = 2200: 518.087 sec elapsed