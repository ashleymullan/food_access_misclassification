...
fix_sigma = 0.8 ## error standard deviation
...
# ---------------------------------------------------------------------------------
sim_data = function(N, beta0 = fix_beta0, beta1 = fix_beta1, muU, sigmaU = fix_sigma, access = fix_access) {
...
for (N in c(390, 2200)) {
  ...
  # And error mean: -1, -0.7, -0.35, -0.1
  for (mu in c(-1, -0.7, -0.35, -0.1)){
    # Be reproducible
    ...
    sett_res = data.frame(
      ...
      N, beta0 = fix_beta0, beta1 = fix_beta1, muU = mu, ## simulation setting
      sigmaU = fix_sigma, q = fix_q, avg_prev = NA, fpr = NA, ## simulation setting
      ...
      beta0_smle = NA, se_beta0_smle = NA, beta1_smle = NA, se_beta1_smle = NA ## imputation analysis
    )
    ...
    for (r in 1:num_reps) {
      ...
      write.csv(x = sett_res,
                file = paste0("vary_muU/proximity_N", N, "_muU", abs(100 * mu), "_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
...
# Timing from tictoc:
## Sims with N = 390: 214.262 sec elapsed
## Sims with N = 2200: 413.857 sec elapsed