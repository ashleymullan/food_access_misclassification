...
# ---------------------------------------------------------------------------------
sim_data = function(N, beta0 = fix_beta0, beta1 = fix_beta1, muU = fix_muU, sigmaU = fix_sigmaU, access) {
...
for (N in c(390, 2200)) {
  ...
  for (access in c(0.5, 1, 5, 10)){
    ...
    sett_res = data.frame(
      ...
      sigmaU = fix_sigmaU, q = fix_q, access = access, avg_prev = NA, fpr = NA, ## simulation setting
      ...
      beta0_smle = NA, se_beta0_smle = NA, beta1_smle = NA, se_beta1_smle = NA, ppv_smle = NA,
      msg = "")
    ...
    for (r in 1:num_reps) {
      ...
      dat = sim_data(N  = N, ## sample size
                     access = access ## mile threshold to discretize X into access/no access
                     ) 
      ...
      if (with(dat, sum(binX == 0 & binXstar == 0, na.rm = TRUE) == 0)) {
        sett_res[r, "msg"] = "All X = 1 in subset"
      } else {
      ...
      write.csv(x = sett_res,
                file = paste0("vary_access/proximity_N", N, "_", access, "mile_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
...
# Timing from tictoc:
## Sims with N = 390: 189.554 sec elapsed
## Sims with N = 2200: 944.538 sec elapsed