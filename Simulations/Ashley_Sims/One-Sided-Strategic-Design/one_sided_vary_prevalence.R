...
repo_string <- "food_access_misclassification/Simulations/Ashley_Sims/One-Sided-Strategic-Design"
...
# ---------------------------------------------------------------------------------
sim_data = function(N, beta0, beta1 = fix_beta1, beta2 = fix_beta2, eta0 = fix_eta0, eta1 = fix_eta1) {
...
for (N in c(390, 2200)) {
  ...
  for (exp_beta0 in c(0.05, 0.10, 0.20, 0.33)){
    print(paste("entering prevalence:", exp_beta0))
    #compute beta0
    beta0 <- log(exp_beta0)
    # Be reproducible
    ...
    sett_res = data.frame(
      ...
      N, beta0 = beta0, beta1 = fix_beta1, beta2 = fix_beta2, q = fix_q,  ## simulation setting
    ...
    for (r in 1:num_reps) {
      ...
      query_rows = which(dat$binXstar == 1, arr.ind = TRUE) |>
        sample(size = ceiling(fix_q * N), replace = FALSE)
      ...
      write.csv(x = sett_res,
                file = paste0("ash_varyB0_N", N, "_beta0", beta0, "_seed", sim_seed, ".csv"),
                row.names = F)
    }
  }
...
files <- all_files[grepl("ash_varyB0", all_files)]
...
write.csv(x = res,
          file = paste0("one_sided_vary_prevalence.csv"),
          row.names = F)
