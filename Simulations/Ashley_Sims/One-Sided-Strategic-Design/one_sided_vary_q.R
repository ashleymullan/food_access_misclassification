  ...
  for (q in c(0.05, 0.1, 0.25, 0.5)){
    ...
    for (r in 1:num_reps) {
      ...
      write.csv(x = sett_res, #                   FIX ME
                file = paste0("ash_varyQ_N", N, "_q", 100 * q, "_seed", sim_seed, ".csv"),
                row.names = F)
    }
  }
...
# Timing from tictoc:
# Sims with N = 390: 348.856 sec elapsed
# Sims with N = 2200: 1117.68 sec elapsed

# Create combined results file
# Read simulation data in from GitHub repo
all_files = paste0(list.files())
files <- all_files[grepl("ash_varyQ", all_files)]
