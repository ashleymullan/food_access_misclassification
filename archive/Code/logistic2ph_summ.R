...
#'
#' @param Y Column name that stores the number of events. This argument is required.
#' @param Size Column name that stores the number of trials. This argument is required.
...
logistic2ph_summ <- function(Y = NULL, Size = NULL, X_unval = NULL, X = NULL, Z = NULL, Bspline = NULL, data = NULL, hn_scale = 1, noSE = FALSE, TOL = 1E-4, MAX_ITER = 1000, verbose = FALSE) {
  if (missing(data)) {
  ...
  # Create vector of predictors for outcome model --------------------
  theta_pred <- c(X, Z)
  # -------------------- Create vector of predictors for outcome model
  ...
  comp_dat_val <- comp_dat_val[, c(Y_unval, theta_pred, Bspline, "k")]
  comp_dat_val <- data.matrix(comp_dat_val)
  ## -------------- Save static (X*,Y*,X,Y,Z) since they don't change
  # 2 (m x n)xd matrices (y=0/y=1) of each (one column per person, --
  # one row per x) --------------------------------------------------
  comp_dat_unval <- cbind(data[rep(seq(from = (n+1), N), times = m), c(Y_unval, setdiff(x = theta_pred, y = X), Bspline)],
                          x_obs_stacked,
                          k = rep(seq(1, m), each = (N - n)),
                          row.names = NULL)
  comp_dat_unval <- comp_dat_unval[, c(Y_unval, theta_pred, Bspline, "k")]
  ...
  while(it <= MAX_ITER & !CONVERGED) {
    ...
    if (any(is.na(new_theta))) {
      suppressWarnings(new_theta <- matrix(glm(formula = theta_formula,
                                               family = "binomial",
  ...
  if(noSE) {
    ...
    ## Calculate pl(theta) -------------------------------------------------
    od_loglik_theta <- logistic2ph_summ_loglik(N = N,
      n = n,
      Y = Y,
      Size = Size,
      X_unval = X_unval,
