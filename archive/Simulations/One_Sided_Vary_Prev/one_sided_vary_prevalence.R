...
repo_string <- "food_access_misclassification/Simulations/One_Sided_Vary_Prev"
  ...
  for (exp_beta0 in c(0.05, 0.10, 0.20, 0.33)){
    ...
    for (r in 1:num_reps) {
      ...
      dat = sim_data(N  = N, ## sample size
                     beta0 = beta0) ## baseline prevalence
      ...
      if (length(query_rows) < ceiling(fix_q * N)) {
        query_rows = c(query_rows, 
                       sample(x = which(dat$binXstar == 0), 
                              size = (ceiling(fix_q * N) - length(which(dat$binXstar == 1))),
                              replace = FALSE))
      }
      ...
      write.csv(x = sett_res, 
                file = paste0("varyPrev_N", N, "_Prev", exp_beta0 * 100, "_seed", sim_seed, ".csv"),
                row.names = F)
    }
  }
...
files <- all_files[grepl("varyPrev", all_files)]
